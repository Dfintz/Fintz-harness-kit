...
describe('eventButtons panel/reminder direct-action routing', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockHandleRemindMe.mockResolvedValue(undefined);
  });

  it('routes event_remindme_* through direct-action path (no deferUpdate branch)', async () => {
    const interaction = {
      customId: 'event_remindme_activity-1',
    ...
    expect(mockHandleRemindMe).toHaveBeenCalledWith(interaction, 'activity-1');
