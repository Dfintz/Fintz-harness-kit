...
import { TOP_LEVEL_SLASH_COMMAND_NAME_SET } from '../constants/slashRoots';
...
// ─── Lazy service accessors for wiki ─────────────────────────
...
/**
 * Build a display line for a command.
 * For commands with subcommand groups, lists the groups.
 * For simple commands, just shows the description.
 */
function formatCommandHelp(cmd: BotCommand): string {
  const json = cmd.data.toJSON();
  const options = json.options ?? [];

  // Find subcommand groups
  const groups = options.filter(o => o.type === ApplicationCommandOptionType.SubcommandGroup);
  // Find direct subcommands (not in groups)
  const directSubs = options.filter(o => o.type === ApplicationCommandOptionType.Subcommand);

  if (groups.length === 0 && directSubs.length === 0) {
    // Simple command (ping, help, etc.)
    return `\`/${json.name}\` — ${json.description}`;
  }

  // Parent command with groups
...
export const help: BotCommand = {
  ...
  handleButton: async (interaction: ButtonInteraction) => {
    const sub = parsePanelCustomId(interaction.customId, 'help');
    if (!sub) {
      return;
    }
    ...
    try {
      switch (sub) {
        ...
        case 'faq_category': {
          ...
          const categoryLabel = new LabelBuilder()
            .setLabel(`Category ID (${botFaqCategories.length} available)`)
        ...
        // ── Original help sub-panels ────────────────────
        case 'server_setup': {
          const embed = new EmbedBuilder()
            .setColor(EmbedColors.SC_BLUE)
            .setTitle('\u2699\ufe0f Server Setup & Admin')
            .setDescription(
              'One-time setup and admin tools.\n' +
        ...
        case 'more_features': {
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              'Additional features accessible through panels.\n' +
                'Click a button to open that feature\u2019s panel.'
            );
          const row1 = new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder()
      ...
      logger.error('Help button handler failed', { error: getErrorMessage(error) });
  ...
  // ── Modal handler for wiki + FAQ modals ──────────────────
  async handleModal(interaction: ModalSubmitInteraction): Promise<void> {
    try {
      ...
      logError(error, 'HelpCommand.handleModal');
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      const commands = interaction.client.commands as Collection<string, BotCommand> | undefined;

      const embed = new EmbedBuilder()
        .setColor(EmbedColors.SC_BLUE)
        .setTitle('Fringe Core Bot \u2014 Help Center')
        .setDescription(
          'Type any command to open its interactive panel with action buttons.\n' +
      ...
      if (commands?.size) {
        // Only show commands that are in the / menu
        const slashCmds = [...commands.values()].filter(cmd =>
          TOP_LEVEL_SLASH_COMMAND_NAME_SET.has(cmd.data.name)
        );
      ...
      await handleCommandError(interaction, error, 'HelpCommand.execute');
    }
  },
};
...
async function routeHelpModal(interaction: ModalSubmitInteraction): Promise<boolean> {
...
async function handleWikiSearchModal(interaction: ModalSubmitInteraction): Promise<void> {
  ...
  const organizationId = await getWikiServices().guildOrgService.resolveOrganization(
    interaction.guildId
  );
  if (!organizationId) {
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `No pages matched **"${truncateText(query, 80)}"**. Try a different search term.`
          ),
      ],
    });
    return;
...
async function handleWikiViewModal(interaction: ModalSubmitInteraction): Promise<void> {
  if (!interaction.guildId) {
    await interaction.reply({
      content: '❌ Wiki can only be used in a server.',
      flags: MessageFlags.Ephemeral,
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: 'Status', value: page.isLocked ? '🔒 Locked' : '📝 Editable', inline: true }
    )
...
async function handleFaqSearchModal(interaction: ModalSubmitInteraction): Promise<void> {
  const query = interaction.fields.getTextInputValue('query').trim();
...
async function handleFaqCategoryModal(interaction: ModalSubmitInteraction): Promise<void> {
