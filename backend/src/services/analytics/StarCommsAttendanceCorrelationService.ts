...
export class StarCommsAttendanceCorrelationService {
  private readonly activityRepository: Repository<Activity>;
  private readonly confirmationRepository: Repository<EventAttendanceConfirmation>;
  ...
  private readonly starCommsAdapter: StarCommsAdapter;

  constructor(
    fleetService: FleetService = new FleetService(),
    integrationService: ExternalIntegrationService = new ExternalIntegrationService(),
    starCommsAdapter: StarCommsAdapter = new StarCommsAdapter()
  ) {
    this.activityRepository = AppDataSource.getRepository(Activity);
    this.confirmationRepository = AppDataSource.getRepository(EventAttendanceConfirmation);
    this.participantRepository = AppDataSource.getRepository(ActivityParticipantEntity);
  ...
  async getReport(
    ...
    const rows = activities.map(activity => {
      ...
      } satisfies CorrelationActivityRow;
    });
  ...
  private async loadActivities(
    organizationId: string,
    filters: StarCommsAttendanceCorrelationFilters
  ): Promise<{ activities: ActivityRow[]; startDate: Date; endDate: Date }> {
    ...
    const query = this.activityRepository
      ...
      .orderBy(
        'COALESCE(activity."scheduledEndDate", activity."scheduledStartDate", activity."createdAt")',
        'ASC'
      );
    ...
    if (filters.activityId) {
      ...
      query
        .andWhere(
          'COALESCE(activity."scheduledEndDate", activity."scheduledStartDate", activity."createdAt") >= :startDate',
          {
            startDate,
          }
        )
        .andWhere(
          'COALESCE(activity."scheduledEndDate", activity."scheduledStartDate", activity."createdAt") <= :endDate',
          {
            endDate,
          }
        );
  ...
  private async loadConfirmations(
    ...
    return this.confirmationRepository
      ...
      .getRawMany<ConfirmationRow>();
  }

  private async loadParticipants(activityIds: string[]): Promise<ParticipantRow[]> {
    if (activityIds.length === 0) {
      return [];
    }
    ...
    return this.participantRepository
      ...
      .getRawMany<ParticipantRow>();
  }

  private async resolveStarCommsSnapshot(
    organizationId: string,
    startDate: Date,
    endDate: Date
  ): Promise<{
    ...
    try {
      ...
      logger.warn('StarComms attendance correlation metrics unavailable', {
        error,
        organizationId,
      });
  ...
  private async findStarCommsIntegration(
    organizationId: string
  ): Promise<(ExternalIntegration & { fleetName: string }) | null> {
  ...
  private groupConfirmations(confirmations: ConfirmationRow[]): Map<string, ConfirmationRow[]> {
  ...
  private groupParticipants(participants: ParticipantRow[]): Map<string, ParticipantRow[]> {
  ...
  private calculateAttendanceStats(confirmations: ConfirmationRow[]): {
    total: number;
    attended: number;
    late: number;
    earlyDeparture: number;
    ...
    for (const confirmation of confirmations) {
      if (confirmation.status === AttendanceStatus.ATTENDED) {
        ...
        stats.late++;
      } else if (confirmation.status === AttendanceStatus.EARLY_DEPARTURE) {
  ...
  private summarizeRoles(participants: ParticipantRow[]): Array<{ role: string; count: number }> {
  ...
  private resolveActivityAnchorDate(activity: ActivityRow): Date {
