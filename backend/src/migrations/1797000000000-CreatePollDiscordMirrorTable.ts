...
/**
 * Sprint 25 — Discord Poll Mirrors
 *
 * Creates poll_discord_mirrors table for tracking poll embeds posted to Discord.
 * Supports org-scoped and federation-scoped poll broadcasting with live updates.
 */
export class CreatePollDiscordMirrorTable1797000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_poll_discord_mirrors_poll" ON "poll_discord_mirrors" ("pollId")`
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_poll_discord_mirrors_guild" ON "poll_discord_mirrors" ("guildId")`
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_poll_discord_mirrors_status" ON "poll_discord_mirrors" ("status")`
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_poll_discord_mirrors_org" ON "poll_discord_mirrors" ("organizationId")`
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_poll_discord_mirrors_poll_guild" ON "poll_discord_mirrors" ("pollId", "guildId")`
    );
  }
