...
/**
 * F-11 Phase 3: Add hybrid encryption mode columns to encrypted_data.
 *
 * Adds:
 *  - encryptionMode ('flat' | 'hybrid')  — existing rows default to 'flat'
 *  - dekId (nullable FK to data_encryption_keys.dekId) — set when mode = 'hybrid'
 *
 * Phase 4 migration status tracking:
 *  - migrationStatus ('none' | 'pending' | 'migrated') — tracks flat→hybrid migration
 */
export class AddHybridModeToEncryptedData1800000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const encryptionModeColumn = await this.resolveColumnName(
      queryRunner,
      'encrypted_data',
      'encryptionMode'
    );
    const dekIdColumn = await this.resolveColumnName(queryRunner, 'encrypted_data', 'dekId');
    const migrationStatusColumn = await this.resolveColumnName(
      queryRunner,
      'encrypted_data',
      'migrationStatus'
    );
    ...
    // 5. Foreign key to data_encryption_keys.dekId
    const dekReferenceColumn = await this.resolveColumnName(
      queryRunner,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "encrypted_data" DROP CONSTRAINT IF EXISTS "FK_encrypted_data_dek"`
    );
