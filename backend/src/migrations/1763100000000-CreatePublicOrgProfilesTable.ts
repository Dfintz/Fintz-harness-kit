...
export class CreatePublicOrgProfilesTable1763100000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Create the public_org_profiles table if it doesn't exist
    ...
    // Create indexes for efficient filtering (skip if they already exist)
    await queryRunner.query(`
            DO $$ BEGIN
            IF NOT EXISTS (
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'IDX_public_org_profiles_memberCount') THEN DROP INDEX "IDX_public_org_profiles_memberCount"; END IF; END $$;`
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'IDX_public_org_profiles_isVerified') THEN DROP INDEX "IDX_public_org_profiles_isVerified"; END IF; END $$;`
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'IDX_public_org_profiles_isRecruiting') THEN DROP INDEX "IDX_public_org_profiles_isRecruiting"; END IF; END $$;`
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'IDX_public_org_profiles_activityLevel') THEN DROP INDEX "IDX_public_org_profiles_activityLevel"; END IF; END $$;`
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'IDX_public_org_profiles_primaryFocus') THEN DROP INDEX "IDX_public_org_profiles_primaryFocus"; END IF; END $$;`
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'IDX_public_org_profiles_isPublic') THEN DROP INDEX "IDX_public_org_profiles_isPublic"; END IF; END $$;`
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_class WHERE relname = 'IDX_public_org_profiles_organizationId') THEN DROP INDEX "IDX_public_org_profiles_organizationId"; END IF; END $$;`
    );
    ...
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'activity_level_enum') THEN DROP TYPE "activity_level_enum"; END IF; END $$;`
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'org_primary_focus_enum') THEN DROP TYPE "org_primary_focus_enum"; END IF; END $$;`
    );
  }
}
