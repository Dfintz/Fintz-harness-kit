...
exports.queryComplexityPlugin = exports.complexityMetrics = exports.ComplexityMetricsCollector = exports.QueryComplexityAnalyzer = void 0;
...
class QueryComplexityAnalyzer {
    ...
    analyze(document, variables = {}, operationName) {
        ...
        const fieldCounts = {};
        const { complexity, depth } = this.calculateComplexity(operation.selectionSet.selections, operation.operation === 'query'
            ? 'Query'
            : operation.operation === 'mutation'
                ? 'Mutation'
                : 'Subscription', fragments, variables, fieldCounts, 0);
    ...
    findOperation(document, operationName) {
        for (const definition of document.definitions) {
    ...
    calculateComplexity(selections, parentType, fragments, variables, fieldCounts, currentDepth) {
        ...
        for (const selection of selections) {
            if (selection.kind === 'Field') {
                ...
                if (selection.selectionSet && config.listMultiplier) {
                    ...
                    const multiplier = limitArg ?? config.listMultiplier ?? this.options.defaultListMultiplier;
                ...
                if (selection.selectionSet) {
                    ...
                    const { complexity, depth } = this.calculateComplexity(selection.selectionSet.selections, childType, fragments, variables, fieldCounts, currentDepth + 1);
                ...
                if (fragment) {
                    const { complexity, depth } = this.calculateComplexity(fragment.selectionSet.selections, fragment.typeCondition.name.value, fragments, variables, fieldCounts, currentDepth);
                    totalComplexity += complexity;
                    maxDepth = Math.max(maxDepth, depth);
                }
            }
                ...
                const { complexity, depth } = this.calculateComplexity(selection.selectionSet.selections, typeName, fragments, variables, fieldCounts, currentDepth);
                totalComplexity += complexity;
                maxDepth = Math.max(maxDepth, depth);
            }
        }
    ...
    findLimitArgument(field, variables) {
        const args = this.extractArguments(field, variables);
        return (args.limit || args.first || args.last || args.perPage);
    }
    guessChildType(fieldName) {
        const knownMappings = {
            fleets: 'Fleet',
            ships: 'Ship',
            members: 'Member',
            ...
            return {
                async didResolveOperation(requestContext) {
                    ...
                    if (options.logComplexity ?? true) {
                        logger_1.logger.debug('GraphQL query complexity', {
                            operationName,
                    ...
                    if (result.exceeds && !skipInDev) {
                        throw new graphql_1.GraphQLError(result.message || 'Query too complex', {
                            extensions: {
...
exports.ComplexityMetricsCollector = ComplexityMetricsCollector;
exports.complexityMetrics = new ComplexityMetricsCollector();
exports.queryComplexityPlugin = createQueryComplexityPlugin({
    maxComplexity: 1000,
    maxDepth: 10,
    logComplexity: true,
    skipInDevelopment: false,
    onComplexity: result => exports.complexityMetrics.record(result),
});
