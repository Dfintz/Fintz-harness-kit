...
const OrganizationArchiveService_1 = require("../../services/organization/OrganizationArchiveService");
const ResourceArchiveService_1 = require("../../services/organization/ResourceArchiveService");
const BaseController_1 = require("../BaseController");
const SUPPORTED_RESOURCE_TYPES = ['organization', 'fleet'];
class ArchiveController extends BaseController_1.BaseController {
    archiveService;
    resourceArchiveService;
    constructor() {
        super();
        this.archiveService = OrganizationArchiveService_1.OrganizationArchiveService.getInstance();
        this.resourceArchiveService = ResourceArchiveService_1.ResourceArchiveService.getInstance();
    }
    list = async (req, res) => {
    ...
    archive = async (req, res) => {
        await this.execute(req, res, async () => {
            ...
            const type = (resourceType || 'organization');
            ...
            if (type === 'fleet') {
                const archived = await this.resourceArchiveService.archiveFleet(resourceId, organizationId, user.id, reason);
                res.status(201).json({
                    success: true,
                    data: {
                        id: archived.id,
    ...
    getById = async (req, res) => {
        await this.execute(req, res, async () => {
            ...
            if (resourceType === 'fleet') {
                const fleet = await this.resourceArchiveService.getArchivedFleetById(archiveId, organizationId);
                if (fleet) {
                    res.json({
                        success: true,
                        data: {
    ...
    restore = async (req, res) => {
        await this.execute(req, res, async () => {
            ...
            if (resourceType === 'fleet') {
                const restored = await this.resourceArchiveService.restoreFleet(archiveId, organizationId, user.id);
                res.json({
                    success: true,
                    data: {
                        id: restored.id,
    ...
    delete = async (req, res) => {
        await this.execute(req, res, async () => {
            ...
            const { resourceType } = req.query;
            if (resourceType === 'fleet') {
                await this.resourceArchiveService.permanentlyDeleteFleet(archiveId, organizationId, user.id);
            }
            else {
    ...
    getStatistics = async (req, res) => {
        await this.execute(req, res, async () => {
            ...
            const archivedFleetCount = await this.resourceArchiveService.getArchivedFleetCount(organizationId);
            res.json({
                success: true,
                data: {
            ...
            for (const record of records || []) {
                try {
                    if (record.resourceType === 'fleet') {
                        await this.resourceArchiveService.archiveFleet(record.resourceId, organizationId, user.id, record.reason);
                    }
                    else {
                        await this.archiveService.archiveOrganization(record.resourceId, user.id, record.reason);
                    }
                    archived++;
                }
                catch (err) {
    ...
    search = async (req, res) => {
        await this.execute(req, res, async () => {
            const organizationId = this.getOrganizationId(req);
            const { q, page, limit, resourceType } = req.query;
            const query = q?.toLowerCase();
            const items = [];
            if (!resourceType || resourceType === 'organization') {
                const archivedOrgs = await this.archiveService.getArchivedOrganizations();
                const filtered = query
                    ? archivedOrgs.filter(org => org.name?.toLowerCase().includes(query) ||
            ...
            if (!resourceType || resourceType === 'fleet') {
                const archivedFleets = await this.resourceArchiveService.getArchivedFleets(organizationId);
                const filtered = query
                    ? archivedFleets.filter(fleet => fleet.name?.toLowerCase().includes(query) ||
