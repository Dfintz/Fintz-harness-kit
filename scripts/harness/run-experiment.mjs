...
function loadLoop(name) {
  ...
  if (!existsSync(path))
    fail(
      `No loop named "${name}" in ${loopsDir}. Use --list to see experiments.`,
    );
  ...
  if (!Number.isInteger(loop.maxIterations) || loop.maxIterations < 1) {
    fail(
      `Loop "${name}" has an invalid maxIterations — experiments must be bounded.`,
    );
  }
  if (
    !loop.metric ||
  ...
  if (!Array.isArray(loop.target) || loop.target.length === 0) {
    fail(
      `Loop "${name}" needs a non-empty target[] (the file(s) the agent optimizes).`,
    );
  }
  return loop;
}
...
function composeImprovementPrompt(loop, iteration, best, current, journal) {
  ...
  const history = journal
    ...
    .map(
      (e) =>
        `- Iteration ${e.iteration}: ${name}=${e.metric} (${e.kept ? "KEPT" : "reverted"}).`,
    );
  return [
    ...
    "",
    ...(history.length > 0
      ? ["## Prior attempts this run", ...history, ""]
      : []),
...
function invokeAgent(agentCmd, prompt, targetFiles) {
  ...
  if (result.status !== 0)
    console.warn(
      `[run-experiment]   agent exited ${result.status}; re-measuring anyway`,
    );
}
...
if (!args.name)
  fail(
    'Usage: run-experiment.mjs <name> [--measure-only] [--max-iterations N] [--agent "<cmd>"]',
  );
...
const commitOnImprove =
  args.commit || process.env.HARNESS_EXPERIMENT_COMMIT === "true";
...
if (targetFiles.length === 0)
  fail(`target ${JSON.stringify(loop.target)} resolved to no files.`);
...
// Write a start-of-run journal so the dashboard sees this run live (status:"running"), not only
// after it finishes. The reader reclassifies a stale running+dead-pid journal as "error".
...
function finish(terminalState, exitCode, message) {
  ...
  writeJournal(journalFile, record);
  // Best-effort transition notification — never let a sink failure change the run's exit.
  try {
    notifyTransition({
      loop: record.loop,
      kind: record.kind,
      state: terminalState,
      metric: record.metric?.best ?? record.metric?.baseline ?? "",
      journal: journalFile,
    });
  } catch {
...
console.log(
  `[run-experiment] baseline ${record.metric.name} = ${best} (goal: ${direction})`,
);
...
for (let iteration = 1; iteration <= maxIterations; iteration++) {
  ...
  const improved =
    measure.value !== null && isImproved(direction, measure.value, best);

  if (improved) {
    best = measure.value;
    noImprovementStreak = 0;
    console.log(
      `[run-experiment]   ${record.metric.name}=${measure.value} — IMPROVED, kept.`,
    );
    if (commitOnImprove) {
      commitTargets(
        targetFiles,
    ...
    // Fire an opt-in "improved" notification so an overnight run signals each kept gain.
    try {
      notifyTransition({
        loop: loop.name,
        kind: "experiment",
  ...
  // Refresh the heartbeat and persist the journal each iteration so the dashboard sees this run
  // live (fresh heartbeatAt) and a mid-run crash leaves a partial journal the reader flags as error.
