...
function parseArgs(argv) {
  const args = { loop: undefined, state: undefined, pass: [], fail: [], help: false };
...
function normalizeRubricItem(raw, where) {
  if (!raw || typeof raw !== 'object') fail(`${where}: each rubric entry must be an object`);
  if (typeof raw.item !== 'string' || raw.item.trim() === '')
    fail(`${where}: rubric "item" must be a non-empty string`);
  if (typeof raw.pass !== 'boolean') fail(`${where}: rubric "${raw.item}" needs a boolean "pass"`);
...
function buildSpec(stdinText, args) {
  ...
  if (args.pass.length || args.fail.length) {
    ...
    spec.rubric = [...(Array.isArray(spec.rubric) ? spec.rubric : []), ...flagRubric];
  }
  return spec;
}
...
if (loopDef?.kind === 'convergence') {
  fail(`"${loopName}" is a convergence loop — record it via run-loop.mjs, not this recorder.`);
}
...
const iterations = rawIterations.map((iteration, index) => {
  ...
  if (rubricRaw.length === 0) fail(`iteration ${index + 1} has no rubric verdicts`);
  const rubric = rubricRaw.map(entry => normalizeRubricItem(entry, `iteration ${index + 1}`));
  return {
    iteration: index + 1,
...
if (loopDef && Array.isArray(loopDef.rubric) && loopDef.rubric.length > 0) {
  ...
  const gradedItems = new Set(iterations.flatMap(it => it.rubric.map(r => r.item)));
  for (const item of gradedItems) {
    if (!declared.has(item))
      console.warn(`[record-run] note: graded criterion not in ${loopName}.rubric: "${item}"`);
  }
  for (const item of declared) {
    if (!gradedItems.has(item))
      console.warn(`[record-run] note: declared criterion not graded this run: "${item}"`);
  }
}
...
const startedAt =
  typeof spec.startedAt === 'string' ? spec.startedAt : (iterations[0]?.at ?? nowIso);
const record = {
  loop: loopName,
  kind: 'workflow',
  startedAt,
...
const outFile = join(runsDir, `${loopName}-${nowIso.replace(/[:.]/g, '-')}.json`);
try {
