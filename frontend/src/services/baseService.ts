...
/**
 * Unwrap apiClient response which may be:
 * - The raw body (when backend doesn't wrap in ApiResponse)
 * - An object with `.data` property (when backend wraps in ApiResponse)
 *
 * apiClient.get/post already returns `response.data` (Axios unwrap),
 * but some backend controllers wrap their payload in `{ data: T }`.
 ...
 * Returns empty array if the response is not an array.
 */
...
/**
 * Extract an array from an API response that may be in multiple shapes:
 * - Direct array: [...]
 * - Wrapped: { data: [...] }
 * - Named field: { templates: [...] } (specify fieldName)
 *
 * This eliminates nested ternaries in service response parsing.
 */
export function extractArrayFromEnvelope<T>(envelope: unknown, fieldName?: string): T[] {
  ...
  // Envelope is non-null but no recognisable array found — log for debugging
 ...
 * Checks common locations: .pagination, .meta, .meta.pagination
 */
